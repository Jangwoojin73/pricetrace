...
1. **전 품목 실시간 최저가 자동 감지**: 신라면, 햇반, 음료, 생수뿐만 아니라 에어팟, 운동화, 생활용품 등 네이버 쇼핑 전체 상품 실시간 크롤링
2. **동적 고화질 이미지 연동**: 네이버 쇼핑 공식 이미지 서버(CDN)와 직결하여 검색된 실제 상품 사진 자동 노출
3. **클린 필터링 & 정렬**: 광고(AD) 상품 및 비정상 가격 자동 제외, 진짜 최저가 오름차순 정렬
4. **패키지 수량 감지 & 1개당 환산가 자동 계산**: 상품명에서 수량(24개, 18개, 6병 등)을 자동 감지하여 1개당 환산 단가 실시간 계산
5. **스마트 목표가 & 특가 알림**: 상품 가격대에 맞춘 목표가 자동 설정 및 `🚨 [특가 감지]` 뱃지/할인율 산출
6. **최근 30일 가격 추이 차트**: 일자별 최저가 변동 라인 차트 및 AI 구매 적기 진단
7. **Vercel 원클릭 배포 완벽 지원**: Serverless Function (`api/index.py`) 및 `vercel.json` 내장으로 무료 호스팅
